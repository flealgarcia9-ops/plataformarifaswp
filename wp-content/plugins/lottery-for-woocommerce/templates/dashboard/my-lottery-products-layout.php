<?php
/**
 * This template is used for displaying the my lottery products layout.
 *
 * This template can be overridden by copying it to yourtheme/lottery-for-woocommerce/dashboard/my-lottery-products-layout.php
 *
...
?>
<div class="lty-my-lottery-products-wrapper lty-data-table-wrapper">
	<?php
	if (!isset($args['post_ids']) || !lty_check_is_array($args['post_ids'])) :
		...
		<table class = "lty-frontend-table lty-dashboard-<?php echo esc_attr(str_replace('_', '-', $current_lottery_menu)); ?>">
			...
			<tbody>
				<?php
				lty_get_template('dashboard/my-lottery-products.php', array(
					'columns' => $columns,
					'post_ids' => $post_ids,
			...
			<?php if ($pagination['page_count'] > 1) : ?>
				<tfoot>
					<tr>
						<td colspan="<?php echo esc_attr(count($columns)); ?>" class="footable-visible" data-action_name="lty_my_lottery_products">
