<?php
/**
 * Admin - Unlimited Scheduled Giveaway Extended.
 *
 * @since 11.7.0
 * */
...
if ( ! class_exists( 'LTY_Admin_Unlimited_Lottery_Extended_Notification' ) ) {
	 ...
	 * @since 11.7.0
	 * */
	class LTY_Admin_Unlimited_Lottery_Extended_Notification extends LTY_Notifications {
		...
		public function __construct() {
			$this->id          = 'admin_unlimited_lottery_extended';
			$this->type        = 'admin';
			$this->title       = __( 'Admin - Unlimited Scheduled Giveaway Extend', 'lottery-for-woocommerce' );
			$this->description = __( 'Send email to admin when the unlimited scheduled giveaway is extended.', 'lottery-for-woocommerce' );
		...
		public function get_default_message() {
			...
			The lottery "{product_name}" has been extended.
		...
		public function is_valid( $product ) {
			return $this->is_enabled() && lty_is_lottery_product( $product ) && $product->is_unlimited_scheduled_lottery();
		}
